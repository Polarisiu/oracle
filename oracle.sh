...
menu() {
    ...
    echo -e "${GREEN}01) 开启 ROOT 登录${RESET}"
    echo -e "${GREEN}02) 一键重装系统${RESET}"
    echo -e "${GREEN}03) 恢复 IPv6${RESET}"
    echo -e "${GREEN}04) 安装保活 Oracle${RESET}"
    echo -e "${GREEN}05) 安装 lookbusy 保活${RESET}"
    echo -e "${GREEN}06) 安装 R 探针${RESET}"
    echo -e "${GREEN}07) y 探长${RESET}"
    echo -e "${GREEN}08) OCI 抢机${RESET}"
    echo -e "${GREEN}09) 计算圆周率${RESET}"
    echo -e "${GREEN}10) 甲骨文救砖${RESET}"
